...
serve(async (req) => {
  ...
  const { platform, name, email, company, details } = await req.json();
      ...
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f8fafc; padding: 32px; border-radius: 12px; max-width: 600px; margin: 0 auto; box-shadow: 0 2px 8px #0001;">
          ...
          <table style="width: 100%; border-collapse: collapse; background: #fff; border-radius: 8px; overflow: hidden;">
            ...
            <tr><td style="padding: 10px 16px; font-weight: bold;">Name</td><td style="padding: 10px 16px;">${name}</td></tr>
            <tr style="background: #f1f5f9;"><td style="padding: 10px 16px; font-weight: bold;">Email</td><td style="padding: 10px 16px;"><a href="mailto:${email}" style="color: #2563eb; text-decoration: underline;">${email}</a></td></tr>
            <tr><td style="padding: 10px 16px; font-weight: bold;">Company</td><td style="padding: 10px 16px;">${company}</td></tr>
            <tr style="background: #f1f5f9;"><td style="padding: 10px 16px; font-weight: bold; vertical-align: top;">Details</td><td style="padding: 10px 16px; white-space: pre-line;">${details}</td></tr>
          </table>
        </div>
      `
    })
